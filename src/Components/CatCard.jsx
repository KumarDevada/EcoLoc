...
const CatCard = ({image , name , link , cardid}) => {
  ...
  return (
    <div className='bg-green-100 md:max-w-[20vw]  shadow-sm p-4 rounded-xl flex flex-col  gap-[2vh] cursor-pointer hover:scale-105 transition-transform hover:bg-green-300' onClick={()=>{(link != "#") ? navigate(link) : ("")}}>
        <div className="w-full max-h-[25vh] flex z-10">
              <img src={image} alt="" className=" object-cover rounded-xl " />
        </div>
        <h1 className='text-md text-slate-500 font-montserrat font-bold'>{name}</h1>
        {(link == "#") ? (
          <button
